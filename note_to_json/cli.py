...
class ProcessingResult:
    ...
    def __init__(self, input_path: Path, success: bool, data: Optional[Dict[str, Any]] = None, 
...
def process_single_file(input_path: Path, input_format: str, no_emoji: bool = False) -> ProcessingResult:
    ...
    try:
        ...
        return ProcessingResult(input_path, False, error=error_msg, error_type=error_type)
        ...
        return ProcessingResult(input_path, False, error=str(e), error_type="unexpected_error")


def retry_failed_file(input_path: Path, original_error: str, no_emoji: bool = False) -> ProcessingResult:
    """Retry processing a failed file with different strategies."""
    if no_emoji:
        print(f"  Retrying {input_path.name} with different strategies...", file=sys.stderr)
    else:
        print(f"  🔄 Retrying {input_path.name} with different strategies...", file=sys.stderr)
    ...
    try:
        ...
        if input_path.suffix.lower() in ['.md', '.markdown', '.txt']:
            ...
            if no_emoji:
                return ProcessingResult(input_path, True, data=parsed_data, error="Retry successful with txt format")
            else:
                return ProcessingResult(input_path, True, data=parsed_data, error="🔄 Retry successful with txt format")
    except Exception:
        pass
    ...
    try:
        ...
        if input_path.suffix.lower() in ['.json']:
            ...
            if no_emoji:
                return ProcessingResult(input_path, True, data=parsed_data, error="Retry successful with json format")
            else:
                return ProcessingResult(input_path, True, data=parsed_data, error="🔄 Retry successful with json format")
    except Exception:
        pass
    ...
    try:
        ...
        if no_emoji:
            return ProcessingResult(input_path, True, data=parsed_data, error="Retry successful with raw text processing")
        else:
            return ProcessingResult(input_path, True, data=parsed_data, error="🔄 Retry successful with raw text processing")
    except Exception:
        pass
    ...
    return ProcessingResult(input_path, False, error=f"All retry strategies failed. Original error: {original_error}", error_type="retry_failed")


def process_stdin(input_format: str, no_emoji: bool = False) -> Tuple[Dict[str, Any], Optional[str]]:
...
def print_progress(current: int, total: int, filename: str, no_emoji: bool = False, start_time: Optional[float] = None):
    ...
    if no_emoji:
        ...
        print(f"Processing [{current}/{total}] {progress_bar} {percentage:.1f}%: {filename}", file=sys.stderr)
    else:
        progress_bar = f"[{'🟦' * int(percentage / 5)}{'⬜' * (20 - int(percentage / 5))}]"
        ...
        print(f"🔄 [{current}/{total}] {progress_bar} {percentage:.1f}%: {filename}{time_info}", file=sys.stderr)
...
def print_summary(results: List[ProcessingResult], no_emoji: bool = False):
    ...
    if no_emoji:
        print(f"\nSummary: {len(successful)} successful, {len(failed)} failed", file=sys.stderr)
    else:
        print(f"\n📊 Summary: {len(successful)} successful, {len(failed)} failed", file=sys.stderr)
    ...
    if failed:
        ...
        if "encoding_error" in error_types:
            if no_emoji:
                print("  • For encoding errors: Re-save files as UTF-8 or use --stdin", file=sys.stderr)
            else:
                print("  💡 For encoding errors: Re-save files as UTF-8 or use --stdin", file=sys.stderr)
        
        if "format_mismatch" in error_types:
            if no_emoji:
                print("  • For format errors: Use --input-format to specify the correct format", file=sys.stderr)
            else:
                print("  💡 For format errors: Use --input-format to specify the correct format", file=sys.stderr)
        
        if "validation_error" in error_types or "missing_required_field" in error_types:
            if no_emoji:
                print("  • For validation errors: Check that files have required title and content", file=sys.stderr)
            else:
                print("  💡 For validation errors: Check that files have required title and content", file=sys.stderr)
        
        if "parsing_error" in error_types:
            if no_emoji:
                print("  • For parsing errors: Ensure files contain valid content (not empty)", file=sys.stderr)
            else:
                print("  💡 For parsing errors: Ensure files contain valid content (not empty)", file=sys.stderr)


def determine_exit_code(results: List[ProcessingResult], missing_files: List[Path]) -> int:
...
Examples:
  ...
  cat data.json | note2json --stdin --input-format json --stdout
    ...
    parser.add_argument(
        "--no-emoji",
        action="store_true",
        help="Disable emoji in status output"
    )
    ...
    for i, input_path_str in enumerate(input_files):
        ...
        if args.verbose:
            print_progress(i + 1, len(input_files), input_path.name, args.no_emoji, start_time)
        ...
        if not input_path.exists():
            ...
            if not args.continue_on_error:
                ...
                print(f"Warning: File not found: {input_path} (continuing...)", file=sys.stderr)
                continue
        ...
        if not result.success and not args.continue_on_error:
            if result.error_type == "format_mismatch":
                ...
                print(f"Error: Encoding issue with {input_path.name}: {result.error}", file=sys.stderr)
            else:
                print(f"Error: Failed to parse {input_path.name}: {result.error}", file=sys.stderr)
            sys.exit(3)
        ...
        if not result.success and args.retry_failed:
            ...
            if retry_result.success:
                ...
                if args.no_emoji:
                    print(f"  ✅ Retry successful for {input_path.name}", file=sys.stderr)
                else:
                    print(f"  ✅ Retry successful for {input_path.name}", file=sys.stderr)
        ...
        if args.output:
            if len(processing_results) > 1:
                ...
                if result.success:
                    ...
                    print(f"{ok_prefix}Parsed: {result.input_path.name} → {output_path.name}")
                else:
                    print(f"Error: Cannot write output for failed parse: {result.input_path.name}", file=sys.stderr)
                    sys.exit(3)
        else:
